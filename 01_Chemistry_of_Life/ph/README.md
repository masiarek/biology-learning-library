# pH / pH (odczyn roztworu)

[← Unit 1 · The Chemistry of Life](../README.md)

| English | Polski |
|---|---|
| **Definition.** pH is a 0–14 scale that measures how acidic or basic a solution is. Below 7 is **acidic** (more H⁺ ions), 7 is **neutral**, above 7 is **basic** (alkaline — more OH⁻ ions). | **Definicja.** pH to skala od 0 do 14, która mierzy, jak kwasowy lub zasadowy jest roztwór. Poniżej 7 roztwór jest **kwasowy** (więcej jonów H⁺), 7 oznacza odczyn **obojętny**, powyżej 7 — **zasadowy** (więcej jonów OH⁻). |
| **How it works.** The more H⁺ ions a solution has, the lower its pH; the more OH⁻ ions, the higher its pH. Each step on the scale is a **10-times** change: pH 3 is ten times more acidic than pH 4, and a hundred times more acidic than pH 5. | **Jak działa.** Im więcej jonów H⁺ w roztworze, tym niższe pH; im więcej jonów OH⁻, tym wyższe. Każdy stopień skali to zmiana **dziesięciokrotna**: pH 3 jest dziesięć razy bardziej kwasowe niż pH 4 i sto razy bardziej niż pH 5. |
| **Example.** Stomach acid ≈ pH 2, black coffee ≈ 5, pure water = 7, human blood ≈ 7.4, soap ≈ 10, drain cleaner ≈ 14. | **Przykład.** Kwas żołądkowy ≈ pH 2, czarna kawa ≈ 5, czysta woda = 7, ludzka krew ≈ 7,4, mydło ≈ 10, udrażniacz do rur ≈ 14. |
| **Why it matters.** Enzymes only work in a narrow pH range — outside it they denature. Your body uses **buffers** to hold blood pH near 7.4; even a small shift is dangerous. That is homeostasis working at the level of chemistry. | **Dlaczego to ważne.** Enzymy działają tylko w wąskim zakresie pH — poza nim ulegają denaturacji. Organizm używa **buforów**, by utrzymać pH krwi blisko 7,4; nawet niewielkie przesunięcie jest groźne. To homeostaza na poziomie chemii. |
| **Common mistake.** "Basic" does not mean "safe" and "acidic" does not mean "dangerous" — a strong base like lye (pH 14) burns skin as badly as a strong acid. What matters is how *far* from neutral you are. | **Częsty błąd.** „Zasadowy" nie znaczy „bezpieczny", a „kwasowy" nie znaczy „groźny" — silna zasada, jak ług (pH 14), poparzy skórę równie mocno jak silny kwas. Liczy się to, jak *daleko* roztwór jest od odczynu obojętnego. |

**Słówka:** solution = roztwór · acidic = kwasowy · basic/alkaline = zasadowy · neutral = obojętny · scale = skala · to measure = mierzyć

---

## AP Biology deep dive

**The math.** pH = −log₁₀[H⁺]. A solution at pH 3 has [H⁺] = 10⁻³ M; that is why one pH unit is a tenfold change in hydrogen-ion concentration. Water itself autoionizes (H₂O ⇌ H⁺ + OH⁻) with [H⁺][OH⁻] = 10⁻¹⁴ at 25 °C, which is where the 0–14 range and the neutrality point [H⁺] = [OH⁻] = 10⁻⁷ come from.

**Buffers, quantitatively.** A buffer is a conjugate weak acid–base pair that soaks up added H⁺ or OH⁻. The one AP expects you to know is the **carbonic acid–bicarbonate** system in blood: CO₂ + H₂O ⇌ H₂CO₃ ⇌ H⁺ + HCO₃⁻. Add acid and the equilibrium shifts left; add base and it shifts right — blood pH stays within 7.35–7.45. This couples chemistry to physiology: the medulla senses pH drops (from dissolved CO₂) and raises breathing rate to vent CO₂, a negative feedback loop.

**Why proteins care.** Ionizable R-groups (carboxyl, amino) change protonation state with pH, remaking the ionic bonds and hydrogen bonds that hold tertiary structure — the mechanistic reason enzymes have sharp pH optima and denature outside them.

**Ecology link.** Ocean acidification: dissolved CO₂ forms carbonic acid, and the drop from pre-industrial pH ≈ 8.2 toward ≈ 8.1 today is ~30% more H⁺ (the log scale hides the size of the change). Lower pH pulls carbonate out of reach of shell-building organisms — a favorite AP synthesis question connecting Unit 1 chemistry to Unit 8 ecology.

**Where it sits in AP.** Water chemistry and pH are **AP Unit 1 (Chemistry of Life)**; the buffer physiology reappears in feedback-loop questions of **Unit 4 (Cell Communication)** and the ecology link in **Unit 8**.

**See also:** [Acid](../acid/README.md) · [Base](../base/README.md) · [Buffer](../buffer/README.md) · [Denatured Enzyme](../denatured_enzyme/README.md) · [Homeostasis](../homeostasis/README.md)
